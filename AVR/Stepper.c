...
void prepare_single(uint8_t index, float angle) {
	steppers[index].steps = angle / steppers[index].deg_per_step;
	steppers[index].interval = 1.0 / (2.0 * steppers[index].speed);
}
...
void set_speed(uint8_t index, float speed) { 			// speed in rpm
	...
	if (speed < 0) steppers[index].speed = 0.0;			// Speed can't be nagative
		...
		speed = speed * 360.0 / 60.0;
		
		// Convert deg/s to steps/us and store value:
		steppers[index].speed = speed / (1000000.0 * steppers[index].deg_per_step);
	}
}
